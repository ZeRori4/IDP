# Напишите здесь описание скрипта
"""
<parameters>
	<title>onboarding 3 </title>
	<version>1.0</version>
<parameter>
    <type>string</type>
    <name>Текст сообщения</name>
    <id>MSG</id>
    <value>Hello, world!</value>
</parameter>
<parameter>
    <type>string_from_list</type>
    <id>TYPE_MSG</id>
    <name>Тип сообщения</name>
    <value>output_msg</value>
    <string_list>output_msg,output_alert,output_error,output_logger,output_raise</string_list>
</parameter>

<resources>
    <resource>helpers.py</resource>
</resources>

</parameters>
"""

resources = {
    "helpers.py": """
        
        OlwTi1NUMONoGQ+csmPZNyCLg2IsNc74OBR4YynBljDA/D9vyTrG
    """,
}
t1utils.resources_check(script_path, resources)


import host
import helpers
DEBUG = True
logger = helpers.init_logger("myLogger2", debug=DEBUG)

GLOBALS = globals()

MSG = GLOBALS.get("MSG", "Hello, world!")
TYPE_MSG = GLOBALS.get("TYPE_MSG", "")

def output_msg_user(MSG, TYPE_MSG):
    if TYPE_MSG == "output_msg":
        return message(MSG)
    if TYPE_MSG == "output_alert":
        return alert(MSG)
    if TYPE_MSG == "output_error":
        return error(MSG)
    if TYPE_MSG == "output_logger":
        return logger.debug(MSG)
    if TYPE_MSG == "output_raise":
        raise Exception(MSG)


output_msg_user(MSG, TYPE_MSG)
