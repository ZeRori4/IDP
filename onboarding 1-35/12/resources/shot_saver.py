...
class TRObject(object):
    ...
    def __init__(self, sett):
        self.sett = sett
        self.name = self.replace_spec_symbols(sett.name)

    def replace_spec_symbols(self, name):
        __new_name = self.spec_symbols.sub("_", name.strip())
        logger.debug("replace spec symbols, old name: %s, new name: %s" % (name, __new_name))
        return __new_name
...
class ThreadPool:
    ...
    def create_workers(self):
        ...
        while len(self.workers) < self.tasks.qsize():
            if len(self.workers) == self.num_threads:
                break
